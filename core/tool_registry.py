...
class ToolRegistry:
    ...
    def _initialize_tool_effectiveness(self) -> Dict[str, Dict[str, float]]:
        ...
        return {
            ...
            TargetType.SOCIAL_MEDIA_INTEL.value: {
                "maigret": 0.95,
                "sherlock": 0.93,
                "blackbird": 0.90,
                "social-analyzer": 0.85,
                "whatsmyname": 0.88,
                "namechk": 0.80,
                "userrecon": 0.78,
                "twint": 0.82,
                "social-searcher": 0.75,
            },
            TargetType.DNS_INTEL.value: {
                "subfinder": 0.95,
                "dnsx": 0.93,
                "dnstwist": 0.90,
                "dnsrecon": 0.88,
    ...
    def _register_reddit_tools(self):
        ...
        self._register(ToolDefinition(
            ...
            target_type=TargetType.REDDIT_INTEL,
            tool_type=ToolType.PYTHON_LIBRARY,
            description="Real-time streaming of Reddit submissions and comments using PRAW streams",
            effectiveness=0.83,
            priority=25,
            python_package="praw",
            install_commands=["pip install praw"],
            requires_auth=True,
            auth_env_vars=["REDDIT_CLIENT_ID", "REDDIT_CLIENT_SECRET"],
            capabilities=["submission_stream", "comment_stream", "real_time_monitor"],
            output_formats=["json"],
            rate_limit=1.0,
        ))
        self._register(ToolDefinition(
            ...
            target_type=TargetType.REDDIT_INTEL,
            tool_type=ToolType.PYTHON_LIBRARY,
            description="Reddit scraper using requests/BS4 — scrape posts, comments, and user profiles without API",
            effectiveness=0.78,
            priority=35,
            python_package="beautifulsoup4",
            install_commands=["pip install beautifulsoup4 requests"],
            capabilities=["post_scrape", "comment_scrape", "user_profile_scrape"],
            output_formats=["json"],
            rate_limit=3.0,
        ))
        self._register(ToolDefinition(
            ...
            target_type=TargetType.REDDIT_INTEL,
            tool_type=ToolType.REST_API,
            description="Reddit user profiler — analyze activity patterns, interests, and posting behavior",
            effectiveness=0.75,
            priority=40,
            api_base_url="https://www.snoopsnoo.com",
            capabilities=["user_analysis", "activity_pattern", "interest_mapping", "subreddit_stats"],
            output_formats=["json"],
            rate_limit=2.0,
        ))
    ...
    def _register_telegram_tools(self):
        ...
        self._register(ToolDefinition(
            ...
            target_type=TargetType.TELEGRAM_INTEL,
            tool_type=ToolType.PYTHON_LIBRARY,
            description="Modern async Telegram MTProto client — fast channel/group monitoring and message extraction",
            effectiveness=0.93,
            priority=12,
            python_package="pyrogram",
            install_commands=["pip install pyrogram tgcrypto"],
            requires_auth=True,
            auth_env_vars=["TELEGRAM_API_ID", "TELEGRAM_API_HASH"],
            auth_description="Telegram API credentials from my.telegram.org",
            capabilities=["channel_monitor", "group_monitor", "message_search", "user_lookup",
                          "media_download", "message_history", "chat_export"],
            output_formats=["json"],
            max_results_per_call=5000,
        ))
        self._register(ToolDefinition(
            ...
            target_type=TargetType.TELEGRAM_INTEL,
            tool_type=ToolType.CLI_BINARY,
            description="Telegram group/channel scraper — extract members, messages, and metadata",
            effectiveness=0.82,
            priority=35,
            cli_command="tgscraper",
            install_commands=["composer global require tgscraper/tgscraper"],
            requires_auth=True,
            auth_env_vars=["TELEGRAM_API_ID", "TELEGRAM_API_HASH"],
            capabilities=["member_scrape", "message_scrape", "channel_metadata"],
            output_formats=["json", "csv"],
        ))
        self._register(ToolDefinition(
            name="telegram-dump",
            target_type=TargetType.TELEGRAM_INTEL,
            tool_type=ToolType.PYTHON_LIBRARY,
            description="Export and dump Telegram channel/group messages and media for offline analysis",
            effectiveness=0.80,
            priority=38,
            python_package="telethon",
            install_commands=["pip install telethon"],
            requires_auth=True,
            auth_env_vars=["TELEGRAM_API_ID", "TELEGRAM_API_HASH"],
            capabilities=["message_dump", "media_export", "channel_archive"],
            output_formats=["json", "csv", "html"],
        ))
    ...
    def _register_paste_leak_tools(self):
        ...
        self._register(ToolDefinition(
            ...
            target_type=TargetType.PASTE_LEAKS,
            tool_type=ToolType.REST_API,
            description="Have I Been Pwned API — check emails and passwords against known data breaches",
            effectiveness=0.95,
            priority=3,
            api_base_url="https://haveibeenpwned.com/api/v3",
            requires_auth=True,
            auth_env_vars=["HIBP_API_KEY"],
            auth_description="HIBP API key from haveibeenpwned.com/API/Key",
            capabilities=["breach_check", "paste_check", "password_check", "domain_search",
                          "breach_catalog"],
            output_formats=["json"],
            rate_limit=1.5,
        ))
        self._register(ToolDefinition(
            ...
            target_type=TargetType.PASTE_LEAKS,
            tool_type=ToolType.REST_API,
            description="Dehashed API — search leaked/breached credentials by email, username, IP, name, address",
            effectiveness=0.92,
            priority=8,
            api_base_url="https://api.dehashed.com",
            requires_auth=True,
            auth_env_vars=["DEHASHED_EMAIL", "DEHASHED_API_KEY"],
            auth_description="Dehashed account email and API key",
            capabilities=["email_search", "username_search", "ip_search", "password_search",
        ...
        self._register(ToolDefinition(
            ...
            target_type=TargetType.PASTE_LEAKS,
            tool_type=ToolType.REST_API,
            description="Snusbase API — search leaked databases for credentials, hashes, and personal info",
            effectiveness=0.88,
            priority=12,
            api_base_url="https://api.snusbase.com",
            requires_auth=True,
            auth_env_vars=["SNUSBASE_API_KEY"],
            auth_description="Snusbase API key from snusbase.com",
            capabilities=["email_search", "username_search", "hash_search", "password_search",
                          "ip_search", "name_search"],
            output_formats=["json"],
            rate_limit=1.0,
        ))
        self._register(ToolDefinition(
            ...
            target_type=TargetType.PASTE_LEAKS,
            tool_type=ToolType.REST_API,
            description="LeakCheck API — check if credentials appeared in known data breaches and leaks",
            effectiveness=0.86,
            priority=15,
            api_base_url="https://leakcheck.io/api/v2",
            requires_auth=True,
            auth_env_vars=["LEAKCHECK_API_KEY"],
            auth_description="LeakCheck API key from leakcheck.io",
            capabilities=["email_check", "username_check", "phone_check", "hash_check",
                          "domain_check"],
            output_formats=["json"],
            rate_limit=1.0,
        ))
        self._register(ToolDefinition(
            ...
            target_type=TargetType.PASTE_LEAKS,
            tool_type=ToolType.REST_API,
            description="LeakLookup API — search leaked databases by email, username, keyword, domain, or hash",
            effectiveness=0.84,
            priority=18,
            api_base_url="https://leak-lookup.com/api",
            requires_auth=True,
            auth_env_vars=["LEAKLOOKUP_API_KEY"],
            auth_description="LeakLookup API key from leak-lookup.com",
            capabilities=["email_search", "username_search", "keyword_search", "domain_search",
                          "hash_search"],
            output_formats=["json"],
            rate_limit=1.0,
        ))
    ...
    def _register_infrastructure_tools(self):
        ...
        self._register(ToolDefinition(
            ...
            target_type=TargetType.INFRASTRUCTURE_INTEL,
            tool_type=ToolType.PYTHON_LIBRARY,
            description="Netlas.io — internet-wide scanner for hosts, DNS, certificates, and whois data",
            effectiveness=0.87,
            priority=22,
            python_package="netlas",
            install_commands=["pip install netlas"],
            requires_auth=True,
            auth_env_vars=["NETLAS_API_KEY"],
            auth_description="Netlas API key from netlas.io",
            capabilities=["host_search", "dns_search", "cert_search", "whois_search",
                          "response_search"],
            output_formats=["json"],
            rate_limit=1.0,
        ))
        self._register(ToolDefinition(
            ...
            target_type=TargetType.INFRASTRUCTURE_INTEL,
            tool_type=ToolType.REST_API,
            description="SecurityTrails API — historical DNS, WHOIS, and domain data intelligence",
            effectiveness=0.90,
            priority=18,
            api_base_url="https://api.securitytrails.com/v1",
            requires_auth=True,
            auth_env_vars=["SECURITYTRAILS_API_KEY"],
            auth_description="SecurityTrails API key from securitytrails.com",
            capabilities=["dns_history", "whois_history", "subdomain_enum", "associated_domains",
                          "ip_history", "company_search"],
            output_formats=["json"],
            rate_limit=1.0,
            max_results_per_call=1000,
        ))
        self._register(ToolDefinition(
            ...
            target_type=TargetType.INFRASTRUCTURE_INTEL,
            tool_type=ToolType.REST_API,
            description="IPinfo — IP address geolocation, ASN, company, carrier, and privacy detection",
            effectiveness=0.82,
            priority=28,
            api_base_url="https://ipinfo.io",
            python_package="ipinfo",
            install_commands=["pip install ipinfo"],
            requires_auth=True,
            auth_env_vars=["IPINFO_API_KEY"],
            auth_description="IPinfo API token from ipinfo.io (free tier: 50K/month)",
            capabilities=["ip_geolocation", "asn_lookup", "company_lookup", "privacy_detection",
                          "carrier_detection", "hosted_domains"],
            output_formats=["json"],
            rate_limit=0.5,
        ))
        self._register(ToolDefinition(
            ...
            target_type=TargetType.INFRASTRUCTURE_INTEL,
            tool_type=ToolType.CLI_BINARY,
            description="Fast HTTP toolkit — probe live hosts, extract titles, status codes, technologies, and certs",
            effectiveness=0.88,
            priority=16,
            cli_command="httpx",
            install_commands=["go install -v github.com/projectdiscovery/httpx/cmd/httpx@latest"],
            capabilities=["http_probe", "title_extraction", "status_code", "tech_detection",
                          "cert_extraction", "content_length", "redirect_chain"],
            output_formats=["json", "txt"],
        ))
    ...
    def _register_social_media_tools(self):
        ...
        self._register(ToolDefinition(
            ...
            target_type=TargetType.SOCIAL_MEDIA_INTEL,
            tool_type=ToolType.CLI_BINARY,
            description="OSINT tool to search usernames across 500+ sites with async requests",
            effectiveness=0.90,
            priority=15,
            python_package="blackbird",
            cli_command="blackbird",
            install_commands=["pip install blackbird"],
            capabilities=["username_search", "social_profile_discovery", "async_scanning"],
            output_formats=["json", "csv"],
        ))
        self._register(ToolDefinition(
            name="whatsmyname",
            target_type=TargetType.SOCIAL_MEDIA_INTEL,
            tool_type=ToolType.PYTHON_LIBRARY,
            description="Username enumeration using the WhatsMyName project's web account database",
            effectiveness=0.88,
            priority=18,
            python_package="whatsmyname",
            install_commands=["pip install whatsmyname"],
            capabilities=["username_search", "web_account_enum", "profile_discovery"],
            output_formats=["json"],
        ))
        self._register(ToolDefinition(
            name="namechk",
            target_type=TargetType.SOCIAL_MEDIA_INTEL,
            tool_type=ToolType.REST_API,
            description="Check username and domain availability across social networks and domains",
            effectiveness=0.80,
            priority=30,
            api_base_url="https://namechk.com",
            capabilities=["username_check", "domain_availability", "social_enum"],
            output_formats=["json"],
            rate_limit=2.0,
        ))
        self._register(ToolDefinition(
            ...
            target_type=TargetType.SOCIAL_MEDIA_INTEL,
            tool_type=ToolType.CLI_BINARY,
            description="Find usernames across 75 social networks for reconnaissance",
            effectiveness=0.78,
            priority=35,
            cli_command="userrecon",
            install_commands=["npm install -g userrecon"],
            capabilities=["username_search", "social_profile_check"],
            output_formats=["json", "txt"],
        ))
        self._register(ToolDefinition(
            name="twint",
            target_type=TargetType.SOCIAL_MEDIA_INTEL,
            tool_type=ToolType.PYTHON_LIBRARY,
            description="Advanced Twitter/X scraping without API — followers, tweets, mentions, hashtags",
            effectiveness=0.82,
            priority=25,
            python_package="twint",
            cli_command="twint",
            install_commands=["pip install twint"],
            capabilities=["tweet_scrape", "user_scrape", "follower_scrape", "hashtag_search",
                          "mention_tracking", "geo_search"],
            output_formats=["json", "csv"],
            rate_limit=2.0,
        ))
        self._register(ToolDefinition(
            ...
            target_type=TargetType.SOCIAL_MEDIA_INTEL,
            tool_type=ToolType.PYTHON_LIBRARY,
            description="Social network scraper for Twitter, Reddit, Instagram, Facebook, VKontakte, and more",
            effectiveness=0.88,
            priority=12,
            python_package="snscrape",
            cli_command="snscrape",
            install_commands=["pip install snscrape"],
            capabilities=["twitter_scrape", "reddit_scrape", "instagram_scrape", "facebook_scrape",
                          "vk_scrape", "telegram_scrape"],
            output_formats=["json"],
        ))
        self._register(ToolDefinition(
            ...
            target_type=TargetType.SOCIAL_MEDIA_INTEL,
            tool_type=ToolType.REST_API,
            description="Social Searcher API — real-time social media search and monitoring across platforms",
            effectiveness=0.75,
            priority=40,
            api_base_url="https://api.social-searcher.com/v2",
            requires_auth=True,
            auth_env_vars=["SOCIAL_SEARCHER_API_KEY"],
            auth_description="Social Searcher API key from social-searcher.com",
            capabilities=["social_search", "mention_monitoring", "sentiment_analysis", "trend_tracking"],
            output_formats=["json"],
            rate_limit=1.0,
        ))
    ...
    def _register_dns_tools(self):
        ...
        self._register(ToolDefinition(
            ...
            target_type=TargetType.DNS_INTEL,
            tool_type=ToolType.CLI_BINARY,
            description="Find related domains and subdomains using certificate transparency and various sources",
            effectiveness=0.86,
            priority=18,
            cli_command="assetfinder",
            install_commands=["go install github.com/tomnomnom/assetfinder@latest"],
            capabilities=["subdomain_enum", "related_domain_discovery", "cert_transparency"],
            output_formats=["txt"],
        ))
        self._register(ToolDefinition(
            name="dnsdumpster",
            target_type=TargetType.DNS_INTEL,
            tool_type=ToolType.REST_API,
            description="DNSDumpster — DNS recon and research, find host records and map attack surface",
            effectiveness=0.83,
            priority=25,
            api_base_url="https://api.hackertarget.com",
            capabilities=["dns_lookup", "subdomain_enum", "host_records", "mx_records", "txt_records"],
            output_formats=["json", "txt"],
            rate_limit=3.0,
        ))
        self._register(ToolDefinition(
            ...
            target_type=TargetType.DNS_INTEL,
            tool_type=ToolType.CLI_BINARY,
            description="Fetch URLs from the Wayback Machine for a given domain — discover historical endpoints",
            effectiveness=0.85,
            priority=22,
            cli_command="waybackurls",
            install_commands=["go install github.com/tomnomnom/waybackurls@latest"],
            capabilities=["url_discovery", "historical_endpoints", "wayback_archive", "parameter_discovery"],
            output_formats=["txt"],
        ))
        self._register(ToolDefinition(
            name="gau",
            target_type=TargetType.DNS_INTEL,
            tool_type=ToolType.CLI_BINARY,
            description="GetAllUrls — fetch known URLs from AlienVault OTX, Wayback Machine, and Common Crawl",
            effectiveness=0.84,
            priority=24,
            cli_command="gau",
            install_commands=["go install github.com/lc/gau/v2/cmd/gau@latest"],
            capabilities=["url_discovery", "historical_urls", "multi_source_enum"],
            output_formats=["txt"],
        ))
        self._register(ToolDefinition(
            name="katana",
            target_type=TargetType.DNS_INTEL,
            tool_type=ToolType.CLI_BINARY,
            description="Next-gen crawling and spidering framework — fast, configurable web crawler by ProjectDiscovery",
            effectiveness=0.87,
            priority=16,
            cli_command="katana",
            install_commands=["go install github.com/projectdiscovery/katana/cmd/katana@latest"],
            capabilities=["web_crawl", "js_crawl", "form_discovery", "endpoint_extraction",
                          "headless_crawl", "scope_control"],
            output_formats=["json", "txt"],
        ))
